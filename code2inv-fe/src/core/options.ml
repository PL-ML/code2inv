...
let extractSSA = ref false

let print_ssa = ref false
let print_boogie = ref false
let print_sygus = ref false
let print_smt2 = ref false
let print_inv = ref false 
let print_c = ref false 
let print_token = ref false 

let simpl_control = ref false 
let junk = ref 0
let seed = ref 0

let entry_id = ref 0
let exit_id = ref 1000000
...
let opts =
  ...
  ("-x", (Arg.Set extract), "Extract graph");
  (* ("-ssa", (Arg.Set extractSSA), "Extract SSA graph"); *)
  ("-ssa", (Arg.Set print_ssa), "Print ssa graph");
  ("-boogie", (Arg.Set print_boogie), "Print boogie");
  ("-c", (Arg.Set print_c), "Print c code");
  ("-token", (Arg.Set print_token), "Print token code");
  ("-sygus", (Arg.Set print_sygus), "Print sygus");
  ("-smt2", (Arg.Set print_smt2), "Print smt2");
  ("-inv", (Arg.Set print_inv), "Print inv track");
  ("-sc", (Arg.Set simpl_control), "simlify control dependency");
  
  ("-junk", (Arg.Int (fun x -> junk := x)), "add junk vars");
  ("-seed", (Arg.Int (fun x -> seed := x)), "random seed");
