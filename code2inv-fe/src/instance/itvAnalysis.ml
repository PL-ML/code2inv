...
let get_locset mem =
  ItvDom.Mem.foldi (fun l v locset ->
    (* XUJIE_DBG      prerr_endline ("get_locset from mem, key: " ^ (Loc.to_string l) ); *)
    locset
    |> PowLoc.add l
...
let ssa_analysis : Global.t -> ExtractSSAGraph.t
  = fun global ->
  ...
  in
  (* XUJIE_DBG let _  = prerr_endline "going to analyze ssa_graph" in  *)
    Analysis.analyze_ssa_graph  spec  global
