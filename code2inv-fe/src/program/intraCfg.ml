...
module Node = struct
  ...
  let id n =
    match n with
    | ENTRY -> !Options.entry_id
    | EXIT -> !Options.exit_id
    | Node id -> id
...
module Cmd = struct
  ...
  let rec get_uses_from_cil_exp : exp -> string BatSet.t -> string BatSet.t
    = fun exp res ->
      match exp with
      | Const c -> res
      | Lval lv ->  BatSet.add (s_lv lv) res
      | UnOp (_, e, _) ->  get_uses_from_cil_exp e res
      | BinOp(_, e1, e2, _) ->
        let res = get_uses_from_cil_exp e1 res in
        let res = get_uses_from_cil_exp e2 res in
        res
      | _ ->  raise (Failure ("unhandled get_uses" ^ (s_exp exp) ))

  let get_uses : t -> string BatSet.t
    = fun c -> 
      begin
        let res = BatSet.empty in
        match c with
        | Cif (e, _, _, _)
        | Cassume (e, _)
        | Cset (_,e,_) -> get_uses_from_cil_exp e res
        | Ccall (_,fexp, params, _) ->
          List.fold_left (fun res e ->
              get_uses_from_cil_exp e res
            ) res params
        | _ -> res
      end
  let get_defs : t -> string BatSet.t
    = fun c ->
      begin
        let res = BatSet.empty in
        match c with
        | Cset (lv, _ , _)
        | Ccall (Some lv, _, _, _) -> BatSet.add (s_lv lv) res
        | _ -> res
      end

  let rec get_consts_from_cil_exp : exp -> string BatSet.t -> string BatSet.t
    = fun exp res ->
      match exp with
      | Const c ->  BatSet.add (s_const c) res
      | Lval lv ->  res
      | UnOp (_, e, _) ->  get_consts_from_cil_exp e res
      | BinOp(_, e1, e2, _) ->
        let res = get_consts_from_cil_exp e1 res in
        let res = get_consts_from_cil_exp e2 res in
        res
      | _ ->  raise (Failure ("unhandled get_uses" ^ (s_exp exp) ))

  let get_consts : t -> string BatSet.t
    = fun c -> 
      begin
        let res = BatSet.empty in
        match c with
        | Cif (e, _, _, _)
        | Cassume (e, _)
        | Cset (_,e,_) -> get_consts_from_cil_exp e res
        | Ccall (_,fexp, params, _) ->
          List.fold_left (fun res e ->
              get_consts_from_cil_exp e res
            ) res params
        | _ -> res
      end
end
...
let get_defs_uses : node -> t -> (string BatSet.t) * (string BatSet.t)
  = fun n g ->
    let cmd = find_cmd n g in
    let defs = Cmd.get_defs cmd in
    let uses = Cmd.get_uses cmd in
    (defs, uses)

let get_consts : node -> t -> string BatSet.t
  = fun n g ->
    let cmd = find_cmd n g in
    let consts = Cmd.get_consts cmd in
    consts
      ...
      fold_node (fun n g ->
          ...
          | Cmd.Cif (e,tb,fb,loc) ->
            ...
            if List.length succs = 2 then (* normal case *)
              ...
              (* figure out s1/s2 corresponds to true/false branches *)
