...
struct
  ...
  =fun global locset mem f ->
    ...
    let initial =
      list_fold (fun node ->
        (* mydbg *)
          (* let cfg_node = InterCfg.Node.get_cfgnode node in
          prerr_endline ("init_access for node : " ^ string_of_int (IntraCfg.Node.id cfg_node));
          prerr_endline "locset:";
          PowLoc.iter (fun x -> prerr_endline (Loc.to_string x)) locset; *)
         
          let access_info = (Sem.accessof ~locset global node f mem) in 
          let x = Sem.Access.Info.to_string access_info in 
          (* let x = Sem.Access.Info.accessof access_info in  *)
          (* let _ = prerr_endline ("access_info: " ^ x)  in  *)
          Access.add_node node access_info
       )  nodes Access.empty
    in
...
(* add a debug output for access *)

(* def/use information should be decided here *)
