...
(** {2 Print } *)
  ...
  (* val to_dataflow           : t -> string *)
  (* val to_datalfow : t -> unit *)
  val to_dataflow       : t -> (node list) * (node * node * string) list
  (* val to_dataflow       : t -> (node list) * (node * node * PowLoc.t) list *)
...
struct
  ...
  (*  let to_dataflow : t-> unit =fun g -> () *)
  (* let to_dataflow : t-> string =fun g -> "xxx" *)
                                           
  let to_dataflow : t -> (node list) * (node * node * string) list
    = fun dug ->
    let nodes = fold_node (fun v nodes -> v :: nodes) dug [] in
    let edges =
      fold_edges (fun src dst edges ->
          let addrset = get_abslocs src dst dug in
          let str_addr = PowLoc.fold (fun addr s -> (Loc.to_string addr)^","^s ) addrset ""                    in
          (src, dst, str_addr) :: edges
        ) dug []
    in
    (nodes, edges)
