...
sig
  ...
  val analyze_ssa_graph : Spec.t -> Global.t -> ExtractSSAGraph.t
...
struct
  ...
  let dump_ssa_graph (access, global, locset) =
    if !Options.extractSSA then
      let _ = ExtractSSAGraph.print_graph (
          SsaDug.construct_ssa (access, global, locset)
        )
      in
      prerr_endline "dump_ssa_graph...";
      exit 0
    else ()
    ...
    if !Options.dug then
      begin
        ...
        if !Options.extract then
          begin
            let (dug_nodes, dug_edges) = DUGraph.to_dataflow dug in
            ExtractGraph.print_graph  global.icfg (dug_nodes, dug_edges)
          end
        else 
          `Assoc
  ...
  let analyze_ssa_graph : Spec.t -> Global.t -> ExtractSSAGraph.t
    = fun spec global ->
      let sem_fun = Sem.run Strong spec in
      let locs = spec.Spec.locset in
      let dom = spec.Spec.premem in 
      let access = AccessAnalysis.perform global locs sem_fun dom in
      let locset = spec.Spec.locset_fs in
      let ssa_graph = SsaDug.construct_ssa (global, access, locset) in
      ssa_graph
  ...
  let perform : Spec.t -> Global.t -> Global.t * Table.t * Table.t
    =fun spec global ->
      ...
      let _ = dump_ssa_graph (global, access, spec.Spec.locset_fs) in
