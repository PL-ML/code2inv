...
class ExprNode(object):
        ...
        if self.name == '&&' or self.name == '||':
            if len(self.children) == 1:
                ...
                for i in range(len(self.children)):
                    st += '( ' + self.children[i].__str__() + ' )'
                    # st += self.children[i].__str__()
                    if i + 1 < len(self.children):
                        st += ' %s ' % self.name
        else:
            assert len(self.children) == 0 or len(self.children) == 2
            if len(self.children):
                st = self.children[0].__str__() + self.name + self.children[1].__str__()
            else:
                st = self.name
        return st
    ...
    def has_internal_implications(self,pg):
        if self.name == "&&" or self.name == "||":
            z3_str = [ x.to_z3() for x in self.children ]
            N = len(self.children)
            for i in range(N-1):
    ...
    def to_py(self):
        if self.name == "&&":
            r = " and ".join( [ x.to_py() for x in self.children] )
            return "( " + r + " )"
            #return "z3.And(" + ','.join( [ x.to_z3() for x in self.children] ) + ")"
        elif self.name == "||":
            r = " or ".join( [ x.to_py() for x in self.children] )
            return "( " + r + " )"
            #return "z3.Or(" + ','.join( [ x.to_z3() for x in self.children] ) + ")"
        elif len(self.children) == 2:
            return "( " + self.children[0].to_py() + " " + self.name + " " + self.children[1].to_py() + " )"
            #return  "(%s %s %s)" % (self.children[0].to_z3(), self.name, self.children[1].to_z3() )
        else:
            assert len(self.children) == 0
            return self.name
