...
if len(sys.argv) != 6: 
    print("usage: ", sys.argv[0], "stoch_z3.txt", "pie_z3.txt", "ice_z3.txt", "rl_z3.txt", "gen_grammar_z3.txt")
    exit()

lb = ['C2I', 'PIE', 'ICE-DT', 'Code2Inv-S', 'Code2Inv-G']
# load data
lss = []
for f in sys.argv[1:]:
    ls = list(map(int, R(f)))
...
#plt.show()
plt.savefig('z3calls.pdf',  bbox_inches='tight')
